...
echo "<form name='form' method='post' action='".$_SERVER['PHP_SELF'].
   "?tab=admin:admin_delete:deletesoil:deletefert:deletetill&year=".$origYear."&month=".$origMonth."&day=".$origDay.
   "&tyear=".$tcurYear."&tmonth=".$tcurMonth."&tday=".$tcurDay."&fieldID=".$origFieldID."&id=".$id."'>";
/*
echo '<input type="hidden" name="oldCrop" value="'.$implement.'">';
...
// START - check if old crop can be deleted first!!!
   if(!$result){
      ...
      echo '<meta http-equiv="refresh" content=0;URL="tillageTable.php?year='.$origYear.'&month='.$origMonth.
        '&day='.$origDay.'&tyear='.$tcurYear.'&tmonth='.$tcurMonth.'&tday='.$tcurDay.'&fieldID='.$_GET['fieldID'].
        '&tab=admin:admin_delete:deletesoil:deletefert:deletetill>';
   }
}
?>

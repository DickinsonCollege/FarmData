...
echo "<form name='form' method='post' action=\"".$SERVER['PHP_SELF'].
   "?tab=admin:admin_delete:deletesoil:deletefert:deletecompost:deletecompostact&year=".$origYear.
   "&month=".$origMonth.  "&day=".$origDay."&tyear=".$tcurYear."&tmonth=".$tcurMonth."&tday=".$tcurDay.
   "&id=".$id.
   "&pileID=".encodeURIComponent($origPileID)."&act=".encodeURIComponent($origAct)."\">";

echo "<H3> Edit Compost Activity Record </H3>";
echo '<br clear="all"/>';
echo '<br clear="all"/>';
...
echo '<label>Compost Activity:&nbsp</label>';
echo '<div class="styled-select"><select name="act" id="act">';
...
if ($_POST['submit']) {
   ...
   echo $sql = "update compost_activity set actDate='".$year."-".$month."-".$day.
   ...
   if(!$result){
      ...
      echo '<meta http-equiv="refresh" content="0;URL=compostActTable.php?year='.$origYear.'&month='.$origMonth.
        '&day='.$origDay.'&tyear='.$tcurYear.'&tmonth='.$tcurMonth.'&tday='.$tcurDay."&pileID=".
        encodeURIComponent($origPileID)."&act=".encodeURIComponent($origAct).

...
echo "<form name='form' method='post' action=\"".$SERVER['PHP_SELF'].
   "?tab=admin:admin_delete:deletesoil:deletefert:deletecompost:deletecomposttemp&year=".$origYear.
...
if ($_POST['submit']) {
   ...
   if(!$result){
      ...
      echo '<meta http-equiv="refresh" content="0;URL=compostTempTable.php?year='.$origYear.'&month='.$origMonth.
        '&day='.$origDay.'&tyear='.$tcurYear.'&tmonth='.$tcurMonth.'&tday='.$tcurDay."&pileID=".
        encodeURIComponent($origPileID).

...
echo "<form class='pure-form pure-form-aligned' name='form' method='post' action=\"".$_SERVER['PHP_SELF'].
   "?tab=admin:admin_delete:deletesales:delete_packing&year=".$origYear."&month=".$origMonth."&day=".$origDay.
   "&tyear=".$tcurYear."&tmonth=".$tcurMonth."&tday=".$tcurDay."&crop_product=".
    encodeURIComponent($origCrop)."&target=".encodeURIComponent($origTarget)."&bringback=".encodeURIComponent($origBack)."&grade=".encodeURIComponent($origGrade)."&id=".$id."\">";
/*
echo '<input type="hidden" name="oldCrop" value="'.$curCrop.'">';
echo '<input type="hidden" name="oldField" value="'.$grade.'">';
*/
echo '<div class="pure-controls">';
echo "<H3> Edit Packing Record </H3>";
echo '</div>';
echo '<br clear="all"/>';
echo '<br clear="all"/>';
...
echo '<div class="pure-control-group">';
echo '<label>Bring Back:&nbsp</label>';
echo '<div class="styled-select"><select name="bringback" id="bringback">';
$thisBringBack = "Yes";
$other			= "No";
if ($bringBack == 0){
	$thisBringBack = "No";
...
if ($_POST['submit']) {
 ...
 //  echo $sql;
   ...
   if(!$result){
      ...
      echo "<meta http-equiv=\"refresh\" content=\"0;URL=packingTable.php?year=".$origYear."&month=".$origMonth.
        		"&day=".$origDay."&tyear=".$tcurYear."&tmonth=".$tcurMonth."&tday=".$tcurDay.
				"&target=".encodeURIComponent($origTarget)."&bringback=".encodeURIComponent($origBack)."&grade=".encodeURIComponent($origGrade).
        		"&tab=admin:admin_delete:deletesales:delete_packing".

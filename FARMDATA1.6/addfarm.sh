...
if ! mysql -u $ADMINUSER -p$ADMINPASS -e ";"; then
    echo "Invalid admin username or password - terminating."
    echo "Check the username and password and then try running this script again."
    exit 1
fi
echo ""
...
while [[ -z "`mysql -u $ADMINUSER -p$ADMINPASS -qfsBe "select schema_name from information_schema.schemata where schema_name='$USERDB'" 2>&1 | grep -v "Warning: Using a password"`" ]]; do
   echo "Database $USERDB does not exist."
   echo "Please check the name of ther users database and re-enter it here: "
   read USERDB
done

echo "Enter the name of the FARMDATA farm information database (for the new farm that you are adding, must be a database that does not already exist): "
read FARMDB
FARMUSER=`tr -cd '[:alnum:]' < /dev/urandom | fold -w10 | head -n1`
FARMPASS=`tr -cd '[:alnum:]' < /dev/urandom | fold -w10 | head -n1`
   
while [[ ! -z "`mysql -u $ADMINUSER -p$ADMINPASS -qfsBe "select schema_name from information_schema.schemata where schema_name='$FARMDB'" 2>&1 | grep -v "Warning: Using a password"`" ]]; do
...
read FIRSTUSER
while [[ ! -z "`mysql -u $ADMINUSER -p$ADMINPASS -qfsBe "use $USERDB; select username from users where username='$FIRSTUSER'" 2>&1 | grep -v "Warning: Using a password"`" ]]; do
